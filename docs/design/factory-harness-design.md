...
- **滞后普遍**：全部仓落后上游 HEAD（追平 0–35/49，中位 ~23）；ADR-011 巡检面 7 件仅 xx-go-sdk 追平；下游自发追平 8 仓为 0（分母口径见设计文档 §1.7）——收敛优先于分发。
...
- 死亡谷形态实证：xx-web-tools 移植 + 重证完成后未合 main（origin/main 无 .factory）。
