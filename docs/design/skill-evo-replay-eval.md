...
4. **复用现有设施**：评估集格式直接用 badcase 模型（`input/` + `expected.md`），
   不新造 JSON 格式；执行一次 headless claude 产报告，打分器确定性对账（零 LLM judge）
...
沿用 `badcase_runner.parse_expected`：
- `## 预期检查输出` 小节内「脚本自动检出：<规则>、<规则>…」→ expected_rules（参与对账）
- 「人工补充：<说明>」→ manual_rules（仅展示，不参与——脚本本就检不出，如拼音语义）
...
```
def load_eval_set(skill, eval_dir, cfg) -> List[Case]
...
```bash
evo.py evolve --skill ddl-guard --eval <dir> [--budget 16] [--dry-run]
# --dry-run：报告评估集构成（case 数/类型/expected 清单）+ 打分器可运行性 + baseline 分数
```
