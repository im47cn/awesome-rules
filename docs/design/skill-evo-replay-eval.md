...
缩写未规范化）+ 人工补充 6 条（拼音、泛化词、复数形式、表名主体、属性级别命名、
内容视角命名）；006 脚本自动检出 9 条注释规则 + 人工补充 2 条（补充信息必要性、
字段与注释对应）。脚本部分构成 badcase_runner 的回归基线，**人工部分（LLM 按
...
- 「人工补充规则：<规则 ID>、<规则 ID>…」→ manual_rules（规则 ID 对齐
  `ddl-manual-rules.md` / `sql-manual-rules.md` 规则名，如「拼音」「泛化词」
  「复数形式」「表名主体」「字段与注释对应」）
