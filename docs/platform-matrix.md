...
|---|---|---|---|---|---|
| Claude Code | `claude plugin marketplace add git@github.com:im47cn/awesome-rules.git` + `claude plugin install awesome-rules@awesome-rules` | `.claude-plugin/plugin.json` + `marketplace.json`（市场注册 source 指向仓库根 `./`） | 无仓侧声明† | plugin.json `skills` 字段引共享 `./skills/`（L10）；描述层声明 SessionStart 自动注入规范索引（L3，机制在 `hooks/`） | 源1:23-24｜源2:.claude-plugin/plugin.json:3,10、marketplace.json:7-13｜源3:39,51 |
