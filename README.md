...
| [Comet](docs/research/comet.md) | Agent Skill Harness 双工作流运行时：pass@k/pass^k 评估分离 / 发布门禁绑定草稿 hash / 三层状态可恢复 → 七项借鉴点全部落地（task-flow 技能、replay 评估三件套、release_guard 证据门、platform-matrix） |
