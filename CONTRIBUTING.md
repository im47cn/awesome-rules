...
- 分支前缀：`feature/`、`hotfix/`
- Commit 格式：`<type>(<scope>): <subject>`（中文，主题行 ≤100 字符）
...
- 新增测试套件时，在 `.factory/factory-local.json` 的 `parallel_gate.segments` 中登记段（套件 cwd/argv 形态，长段加 `"intra": "auto"`；编排见 `.factory/factory_lib.py` parallel-gate，ADR-016）
...
├── frontend-standards.md             # 通用：前端工程技术规范（Vue3 管理端）
├── api-contract-freeze-standards.md  # 通用：API 契约冻结规范（先冻结再实现）
