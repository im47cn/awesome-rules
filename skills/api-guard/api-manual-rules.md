...
以下规则 `api_check.py` 无法自动检查，审查业务接口时需人工判断。脚本自动检查路径命名（kebab-case）、动作收敛（末段动词集）、路径变量（禁止 path 传标识）、时间注解；四段式结构、统一 POST、版本段属对外 Open API 规范（业务接口不强制以 domain 开头、不强制带版本号），不在本技能检查范围。
