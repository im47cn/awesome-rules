...
> 定位：聚合报告的**默认交付物**（2026-08-26 由「用户要求才画」转为默认
> 输出）——代码审查默认输出时序图，把技术风险点以编号 Note 钉在确切
> 调用处；纯文档/配置类无调用交互的 diff 在报告「未覆盖」声明免图，不硬造。
> 仍是 SKILL.md 两轴流程的**输出增强**，不是替代——图上的每条风险标注
> 仍须来自两轴发现或证据链核查。
...
实测对照（gtsp-xx-service 审查，2026-08-24）：
...
## 画法约定
- 审查时序图必须把发现的缺陷/风险点以编号标注（如 ⚠R1、⚠R2）钉在对应交互箭头上，阻断级风险用颜色块（rect）高亮；图后附图例表：编号 | 级别 | 位置 | 缺陷/风险 | 证据（file:line），图上标注与报告发现保持一一对应
...
## Mermaid 语法坑（实测）
- sequenceDiagram 消息文本禁止使用分号 `;`——它是语句终止符，会把消息截断并报 `Parse error ... Expecting 'SOLID_ARROW'`；一条消息内需分隔内容时改用 `<br/>`
...
## 代码更新后的图维护
- 图更新后不得目测放行：用本地 HTML + CDN mermaid 实测解析与渲染（要求 PARSE OK + RENDER OK），并断言参与者数量齐全、无字面 `<br` 渲染残留后再交付
