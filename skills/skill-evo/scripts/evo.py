...
def cmd_run(args) -> int:
    ...
    if not cfg["enabled"]:
        return 0
    repo = C.repo_root()
    state_path = C.base_paths(cfg)["state"]
    state = S.load_state(state_path)

    targets: list[S.Session] = []
    # 1) 本会话（CC，来自 hook stdin JSON）
    hook_json = {}
    if args.hook_json_file:
        try:
            ...
            try:
                ...
                pass
    transcript = args.transcript or hook_json.get("transcript_path")
    if transcript and Path(transcript).is_file():
        sess = S.parse_cc_session(Path(transcript))
        sess.session_id = hook_json.get("session_id") or sess.session_id
    ...
    for sess in targets:
        ...
        # 处理过即记账（含 error，防失败会话反复重试；no_signal 同样不重提）
        state.setdefault("processed", {})[sess.key] = sess.mtime
...
def main() -> int:
    ...
    p_run = sub.add_parser("run", help="hook 后台入口：总结 CC 会话 + 搭车 omp 扫描")
    p_run.add_argument("--hook-json-file", help="hook stdin JSON 落盘路径")
    p_run.add_argument("--transcript", help="调试：直接指定 transcript 路径（CC 格式）")
