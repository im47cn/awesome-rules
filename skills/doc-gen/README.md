...
| 多项目聚合 | 已迁移至独立仓架构鹰眼 `~/sources/arch-hawkeye`，doc-gen 专注单项目 | ➡️ |
...
（独立仓 `~/sources/arch-hawkeye`，2026-09 拆分），以 `doc-manifest/` 为唯一交接物
（契约见其仓 `AH-MANIFEST.md`；鹰眼经 RULES_PATH 定位本仓的渲染器与 CLI）。
