...
- 一致 → exit 0 且报 10/10；缺失/漂移 → 逐件点名 + exit 1（可挂 CI）
- 旧版实况（3 脚本 + 2 根配置）→ 按 10 件全集报告缺失，不误报已装件
- 非交互零副作用：不 mkdir、不写 ~/.gitmessage
- 在 node/npm 检测前短路：PATH 无 node 仍可巡检（负控制）

分发件清单在此硬编码为 DIST 镜像（与 install.sh DIST 单源对账）——
上游 DIST 静默缩水时，用例②点不全 10 件即红，防止门禁能力悄悄降级。
"""
...
# (src 相对仓库根, dst 相对目标项目根) —— install.sh DIST 的镜像清单（10 件）
DIST = [
...
# issue 实况：旧版已装项目（gtsp-wop-* 三仓）仅 5 件，缺 5 件
...
def test_all_consistent_exits_zero_with_10_of_10(tmp_path):
...
def test_empty_target_names_all_ten_missing(tmp_path):
    target = tmp_path / "proj"
    target.mkdir()
    r = _check(target, env=_fake_home_env(tmp_path))
    assert r.returncode == 1
    missing = {ln.split()[-1] for ln in r.stdout.splitlines() if ln.startswith("缺失")}
    assert missing == set(DIST_SRC)  # 10 件全部点名（硬编码镜像防 DIST 缩水）


def test_legacy_project_reports_missing_five_not_installed_five(tmp_path):
    target = tmp_path / "proj"
    target.mkdir()
    ...
    assert r.returncode == 1
    missing = {ln.split()[-1] for ln in r.stdout.splitlines() if ln.startswith("缺失")}
    # 按 10 件全集报缺失（旧版少件即多件缺失），已装 5 件不误报
