...
- 一致 → exit 0 且报 N/N（N=len(DIST)）；缺失/漂移 → 逐件点名 + exit 1（可挂 CI）
- 旧版实况（3 脚本 + 2 根配置）→ 按 DIST 全集报告缺失，不误报已装件
- 非交互零副作用：不 mkdir、不写 ~/.gitmessage
- 在 node/npm 检测前短路：PATH 无 node 仍可巡检（负控制）

分发件清单在此硬编码为 DIST 镜像（与 install.sh DIST 单源对账）——
上游 DIST 静默缩水时，用例②点不全即红，防止门禁能力悄悄降级。
"""
...
# (src 相对仓库根, dst 相对目标项目根) —— install.sh DIST 的镜像清单；
# 增删分发件时与 install.sh 的 DIST 数组同改（两处是仅有的必改点，计数一律推导）
DIST = [
    ...
    ("tools/git/lefthook/pre-push-delete-guard.sh", ".lefthook/pre-push-delete-guard.sh"),
...
# 按 .cjs 新口径：旧装 .js 不匹配任何分发名 → 缺 = DIST 中未被 LEGACY_INSTALLED
# 按名匹配的件（lefthook.yml 与 3 个同名脚本），且旧 .js 触发「遗留」检出
LEGACY_MISSING = tuple(dst for _, dst in DIST if dst not in set(LEGACY_INSTALLED))
...
def test_all_consistent_exits_zero_with_full_dist(tmp_path):
    target = tmp_path / "proj"
    target.mkdir()
    _install(DIST_SRC, target)
    r = _check(target, env=_fake_home_env(tmp_path))
    assert r.returncode == 0
    assert f"{len(DIST)}/{len(DIST)}" in r.stdout


# ── 负道：缺失/漂移逐件点名，exit 1 可挂 CI ─────────────────────────────────


def test_empty_target_names_all_dist_missing(tmp_path):
    target = tmp_path / "proj"
    target.mkdir()
    r = _check(target, env=_fake_home_env(tmp_path))
    assert r.returncode == 1
    missing = {ln.split()[-1] for ln in r.stdout.splitlines() if ln.startswith("缺失")}
    assert missing == set(DIST_SRC)  # 全部分发件点名（硬编码镜像防 DIST 缩水）


def test_legacy_project_reports_dist_gaps_and_flags_legacy_js(tmp_path):
    target = tmp_path / "proj"
    target.mkdir()
    ...
    assert r.returncode == 1
    missing = {ln.split()[-1] for ln in r.stdout.splitlines() if ln.startswith("缺失")}
    # 按 DIST 全集报缺失（旧版 .js 不匹配新分发名即多件缺失），同名已装件不误报
