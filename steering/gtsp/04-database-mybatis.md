...
> 适用：PO/Mapper/XML、分页插件。表结构设计（字段类型、索引、命名）见 [`../database-design-specification.md`](../database-design-specification.md)。
...
- 日期注解按层次区分：**PO 不加任何日期格式化注解**；**DTO 标 `@JsonFormat(pattern = "yyyy-MM-dd'T'HH:mm:ss.SSSXXX", timezone = "+08:00")`**（ISO 8601 带时区，与 [`../openapi-standards.md`](../openapi-standards.md) 一致，内外零转换；显式 `timezone` 避免依赖 JVM 默认时区）
