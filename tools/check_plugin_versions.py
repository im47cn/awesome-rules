...
退出码: 0=通过 1=版本漂移/清单漂移 2=结构性错误（非 git 仓/缺 package.json）
"""
...
def _tracked_files(root: Path) -> list[str]:
    ...
    try:
        ...
        print(f"check_plugin_versions: git ls-files 失败（非 git 仓?）: {root}", file=sys.stderr)
...
def main() -> int:
    ...
    if not pkg_path.is_file():
        ...
        return 2
    try:
        anchor = json.loads(pkg_path.read_text(encoding="utf-8"))["version"]
    except (json.JSONDecodeError, KeyError) as exc:
        print(f"check_plugin_versions: package.json 解析失败: {exc}", file=sys.stderr)
        return 2
    ...
    for rel, vpath in PLUGIN_MANIFESTS:
        ...
        try:
            ...
            failures.append(f"{rel}: JSON 解析失败: {exc}")
            continue
        for key in vpath:
