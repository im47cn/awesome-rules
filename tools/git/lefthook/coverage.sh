...
cd "$REPO" || { echo "✗ [cov] 无法进入仓库根 $REPO"; exit 1; }
...
if [ "$HAS_JAVA" = 1 ]; then
  ...
  for d in $jv_dirs; do
    ...
    if [ "$MODE" = "light" ]; then
      (
        cd "$d" || exit 0
        # shellcheck disable=SC2035  # glob 结果可能以 - 开头, -- 终止选项解析
        ...
        # shellcheck disable=SC2086  # 有意分词: xmls 含多个空格分隔的 xml 路径
      ...
      (
        ...
        # shellcheck disable=SC2086  # 有意分词: 多个空格分隔的 xml 路径
