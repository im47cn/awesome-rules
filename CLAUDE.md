...
- 测试突现不明失败时，先排查并行进程对仓库的篡改，再归因自己的改动：核对 `git config core.bare` 是否被置 true、有无未知 worktree（`git worktree list`）。2026-08 实证：.factory 流水线把主仓置为 bare，hermetic 套件 4 例连带失败，恢复 false 后 4/4 复绿，与当次 diff 无关。
