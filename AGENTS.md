...
- **通用设计规范**（设计阶段）：`steering/*.md` —— API 设计、数据库设计、测试、Git 提交、审查报告输出、跨仓契约兼容性、任务包与派发守护、前端工程技术规范（Vue3 管理端）、API 契约冻结
...
- `doc-gen` — DDD 单项目文档站生成；多项目聚合归独立仓 `~/sources/arch-hawkeye`（全局观测与治理工程，契约见其 `AH-MANIFEST.md`）
